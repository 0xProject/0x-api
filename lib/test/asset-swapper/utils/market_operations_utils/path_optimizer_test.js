"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const chai = require("chai");
const _ = require("lodash");
require("mocha");
const asset_swapper_1 = require("../../../../src/asset-swapper");
const path_optimizer_1 = require("../../../../src/asset-swapper/utils/market_operation_utils/path_optimizer");
const types_1 = require("../../../../src/asset-swapper/types");
const utils_1 = require("@0x/utils");
const chai_setup_1 = require("../chai_setup");
const constants_1 = require("../../../../src/asset-swapper/utils/market_operation_utils/constants");
chai_setup_1.chaiSetup.configure();
const expect = chai.expect;
const ZERO = new utils_1.BigNumber(0);
const NO_OP_FEE_SCHEDULE = Object.fromEntries(Object.values(types_1.ERC20BridgeSource).map((source) => [source, _.constant({ gas: 0, fee: ZERO })]));
const NO_OP_PATH_PENALTY_OPTS = {
    outputAmountPerEth: ZERO,
    inputAmountPerEth: ZERO,
    exchangeProxyOverhead: () => ZERO,
};
describe('PathOptimizer', () => {
    describe('Single source type (DEX, MultiHop, Native) routing', () => {
        it('Returns the best single DEX source path', async () => {
            const pathOptimizer = new path_optimizer_1.PathOptimizer({
                pathContext: {
                    side: asset_swapper_1.MarketOperation.Sell,
                    inputToken: 'fake-input-token',
                    outputToken: 'fake-output-token',
                },
                chainId: asset_swapper_1.ChainId.Mainnet,
                feeSchedule: NO_OP_FEE_SCHEDULE,
                neonRouterNumSamples: 5,
                fillAdjustor: new asset_swapper_1.IdentityFillAdjustor(),
                pathPenaltyOpts: NO_OP_PATH_PENALTY_OPTS,
                inputAmount: constants_1.ONE_ETHER.times(4),
            });
            // UniswapV2 is the best path.
            const path = pathOptimizer.findOptimalPathFromSamples([
                createDexSamples({
                    source: types_1.ERC20BridgeSource.UniswapV2,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [2, 4, 6, 8],
                }),
                createDexSamples({
                    source: types_1.ERC20BridgeSource.SushiSwap,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [1, 2, 3, 4],
                }),
            ], [], []);
            if (path === undefined) {
                expect(path).to.be.not.undefined();
                return;
            }
            expect(path.fills).lengthOf(1);
            const fill = path === null || path === void 0 ? void 0 : path.fills[0];
            expect(fill.source).eq(types_1.ERC20BridgeSource.UniswapV2);
            expect(fill.input).bignumber.eq(constants_1.ONE_ETHER.times(4));
            expect(fill.output).bignumber.eq(constants_1.ONE_ETHER.times(8));
        });
        it('Returns the multihop path', async () => {
            const pathOptimizer = new path_optimizer_1.PathOptimizer({
                pathContext: {
                    side: asset_swapper_1.MarketOperation.Sell,
                    inputToken: 'fake-input-token',
                    outputToken: 'fake-output-token',
                },
                chainId: asset_swapper_1.ChainId.Mainnet,
                feeSchedule: NO_OP_FEE_SCHEDULE,
                neonRouterNumSamples: 5,
                fillAdjustor: new asset_swapper_1.IdentityFillAdjustor(),
                pathPenaltyOpts: NO_OP_PATH_PENALTY_OPTS,
                inputAmount: constants_1.ONE_ETHER.times(4),
            });
            // Multihop is the best path.
            const path = pathOptimizer.findOptimalPathFromSamples([
                createDexSamples({
                    source: types_1.ERC20BridgeSource.UniswapV2,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [2, 4, 6, 8],
                }),
            ], [
                createMultihopSamples({
                    firstHopSource: types_1.ERC20BridgeSource.Curve,
                    secondHopSource: types_1.ERC20BridgeSource.CurveV2,
                    inputsInEther: [2, 4],
                    outputsInEther: [4, 9],
                }),
            ], []);
            if (path === undefined) {
                expect(path).to.be.not.undefined();
                return;
            }
            expect(path.fills).lengthOf(1);
            const fill = path === null || path === void 0 ? void 0 : path.fills[0];
            expect(fill.source).eq(types_1.ERC20BridgeSource.MultiHop);
            expect(fill.input).bignumber.eq(constants_1.ONE_ETHER.times(4));
            expect(fill.output).bignumber.eq(constants_1.ONE_ETHER.times(9));
        });
        it('Returns the native source path', async () => {
            const pathOptimizer = new path_optimizer_1.PathOptimizer({
                pathContext: {
                    side: asset_swapper_1.MarketOperation.Sell,
                    inputToken: 'fake-input-token',
                    outputToken: 'fake-output-token',
                },
                chainId: asset_swapper_1.ChainId.Mainnet,
                feeSchedule: NO_OP_FEE_SCHEDULE,
                neonRouterNumSamples: 5,
                fillAdjustor: new asset_swapper_1.IdentityFillAdjustor(),
                pathPenaltyOpts: NO_OP_PATH_PENALTY_OPTS,
                inputAmount: constants_1.ONE_ETHER.times(4),
            });
            // Native order is the best route.
            const path = pathOptimizer.findOptimalPathFromSamples([
                createDexSamples({
                    source: types_1.ERC20BridgeSource.UniswapV3,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [1, 2, 3, 4],
                }),
            ], [
                createMultihopSamples({
                    firstHopSource: types_1.ERC20BridgeSource.Curve,
                    secondHopSource: types_1.ERC20BridgeSource.CurveV2,
                    inputsInEther: [4],
                    outputsInEther: [4],
                }),
            ], [
                createOtcOrder({
                    inputInEther: 4,
                    outputInEther: 5,
                }),
            ]);
            if (path === undefined) {
                expect(path).to.be.not.undefined();
                return;
            }
            expect(path.fills).lengthOf(1);
            const fill = path === null || path === void 0 ? void 0 : path.fills[0];
            expect(fill.source).eq(types_1.ERC20BridgeSource.Native);
            expect(fill.input).bignumber.eq(constants_1.ONE_ETHER.times(4));
            expect(fill.output).bignumber.eq(constants_1.ONE_ETHER.times(5));
        });
    });
    describe('Multiplex routing', () => {
        it('Returns the best multiplex path (two DEX samples)', async () => {
            const pathOptimizer = new path_optimizer_1.PathOptimizer({
                pathContext: {
                    side: asset_swapper_1.MarketOperation.Sell,
                    inputToken: 'fake-input-token',
                    outputToken: 'fake-output-token',
                },
                chainId: asset_swapper_1.ChainId.Mainnet,
                feeSchedule: NO_OP_FEE_SCHEDULE,
                neonRouterNumSamples: 5,
                fillAdjustor: new asset_swapper_1.IdentityFillAdjustor(),
                pathPenaltyOpts: NO_OP_PATH_PENALTY_OPTS,
                inputAmount: constants_1.ONE_ETHER.times(4),
            });
            // The best route involves utilizing both.
            const path = pathOptimizer.findOptimalPathFromSamples([
                createDexSamples({
                    source: types_1.ERC20BridgeSource.UniswapV2,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [2, 4, 6, 7],
                }),
                createDexSamples({
                    source: types_1.ERC20BridgeSource.SushiSwap,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [2, 3, 4, 5],
                }),
            ], [], []);
            if (path === undefined) {
                expect(path).to.be.not.undefined();
                return;
            }
            expect(path.fills).lengthOf(2);
            const fill0 = path.fills[0];
            const fill1 = path.fills[1];
            expect(fill0.source).eq(types_1.ERC20BridgeSource.UniswapV2);
            expect(fill1.source).eq(types_1.ERC20BridgeSource.SushiSwap);
            const inputTotal = fill0.input.plus(fill1.input);
            const outputTotal = fill0.output.plus(fill1.output);
            expect(inputTotal).bignumber.eq(constants_1.ONE_ETHER.times(4));
            expect(outputTotal).bignumber.eq(constants_1.ONE_ETHER.times(8));
        });
        it('Returns the best multiplex path (two native orders)', async () => {
            const pathOptimizer = new path_optimizer_1.PathOptimizer({
                pathContext: {
                    side: asset_swapper_1.MarketOperation.Sell,
                    inputToken: 'fake-input-token',
                    outputToken: 'fake-output-token',
                },
                chainId: asset_swapper_1.ChainId.Mainnet,
                feeSchedule: NO_OP_FEE_SCHEDULE,
                neonRouterNumSamples: 5,
                fillAdjustor: new asset_swapper_1.IdentityFillAdjustor(),
                pathPenaltyOpts: NO_OP_PATH_PENALTY_OPTS,
                inputAmount: constants_1.ONE_ETHER.times(4),
            });
            // The best route involves utilizing two and native orders.
            const path = pathOptimizer.findOptimalPathFromSamples([
                createDexSamples({
                    source: types_1.ERC20BridgeSource.UniswapV3,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [1, 2, 3, 4],
                }),
            ], [], [
                createOtcOrder({
                    inputInEther: 2,
                    outputInEther: 4,
                }),
                createOtcOrder({
                    inputInEther: 2,
                    outputInEther: 4,
                }),
            ]);
            if (path === undefined) {
                expect(path).to.be.not.undefined();
                return;
            }
            expect(path.fills).lengthOf(2);
            const fill0 = path.fills[0];
            const fill1 = path.fills[1];
            expect(fill0.source).eq(types_1.ERC20BridgeSource.Native);
            expect(fill1.source).eq(types_1.ERC20BridgeSource.Native);
            const inputTotal = fill0.input.plus(fill1.input);
            const outputTotal = fill0.output.plus(fill1.output);
            expect(inputTotal).bignumber.eq(constants_1.ONE_ETHER.times(4));
            expect(outputTotal).bignumber.eq(constants_1.ONE_ETHER.times(8));
        });
        it('Returns the best multiplex path (DEX + native order)', async () => {
            const pathOptimizer = new path_optimizer_1.PathOptimizer({
                pathContext: {
                    side: asset_swapper_1.MarketOperation.Sell,
                    inputToken: 'fake-input-token',
                    outputToken: 'fake-output-token',
                },
                chainId: asset_swapper_1.ChainId.Mainnet,
                feeSchedule: NO_OP_FEE_SCHEDULE,
                neonRouterNumSamples: 5,
                fillAdjustor: new asset_swapper_1.IdentityFillAdjustor(),
                pathPenaltyOpts: NO_OP_PATH_PENALTY_OPTS,
                inputAmount: constants_1.ONE_ETHER.times(4),
            });
            // The best route involves utilizing both Uniswap V2 and NativeOrder
            const path = pathOptimizer.findOptimalPathFromSamples([
                createDexSamples({
                    source: types_1.ERC20BridgeSource.UniswapV2,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [2, 4, 5, 7],
                }),
            ], [], [
                createOtcOrder({
                    inputInEther: 2,
                    outputInEther: 4,
                }),
            ]);
            if (path === undefined) {
                expect(path).to.be.not.undefined();
                return;
            }
            expect(path.fills).lengthOf(2);
            const fill0 = path.fills[0];
            const fill1 = path.fills[1];
            expect(fill0.source).eq(types_1.ERC20BridgeSource.UniswapV2);
            expect(fill1.source).eq(types_1.ERC20BridgeSource.Native);
            const inputTotal = fill0.input.plus(fill1.input);
            const outputTotal = fill0.output.plus(fill1.output);
            expect(inputTotal).bignumber.eq(constants_1.ONE_ETHER.times(4));
            expect(outputTotal).bignumber.eq(constants_1.ONE_ETHER.times(8));
        });
        it('Returns the best multiplex path (DEX + multihop)', async () => {
            const pathOptimizer = new path_optimizer_1.PathOptimizer({
                pathContext: {
                    side: asset_swapper_1.MarketOperation.Sell,
                    inputToken: 'fake-input-token',
                    outputToken: 'fake-output-token',
                },
                chainId: asset_swapper_1.ChainId.Mainnet,
                feeSchedule: NO_OP_FEE_SCHEDULE,
                neonRouterNumSamples: 5,
                fillAdjustor: new asset_swapper_1.IdentityFillAdjustor(),
                pathPenaltyOpts: NO_OP_PATH_PENALTY_OPTS,
                inputAmount: constants_1.ONE_ETHER.times(4),
            });
            // The best route involves utilizing both Uniswap V2 and MultiHop.
            const path = pathOptimizer.findOptimalPathFromSamples([
                createDexSamples({
                    source: types_1.ERC20BridgeSource.UniswapV2,
                    inputsInEther: [1, 2, 3, 4],
                    outputsInEther: [2, 4, 5, 7],
                }),
            ], [
                createMultihopSamples({
                    firstHopSource: types_1.ERC20BridgeSource.Curve,
                    secondHopSource: types_1.ERC20BridgeSource.CurveV2,
                    inputsInEther: [2, 4],
                    outputsInEther: [4, 7],
                }),
            ], []);
            if (path === undefined) {
                expect(path).to.be.not.undefined();
                return;
            }
            expect(path.fills).lengthOf(2);
            const fill0 = path.fills[0];
            const fill1 = path.fills[1];
            expect(fill0.source).eq(types_1.ERC20BridgeSource.UniswapV2);
            expect(fill1.source).eq(types_1.ERC20BridgeSource.MultiHop);
            const inputTotal = fill0.input.plus(fill1.input);
            const outputTotal = fill0.output.plus(fill1.output);
            expect(inputTotal).bignumber.eq(constants_1.ONE_ETHER.times(4));
            expect(outputTotal).bignumber.eq(constants_1.ONE_ETHER.times(8));
        });
    });
});
function createDexSample(source, inputInEther, outputInEther) {
    return {
        source,
        input: constants_1.ONE_ETHER.times(inputInEther),
        output: constants_1.ONE_ETHER.times(outputInEther),
        fillData: {},
    };
}
function createDexSamples(params) {
    const { source, inputsInEther, outputsInEther } = params;
    return inputsInEther.map((inputInEther, i) => createDexSample(source, inputInEther, outputsInEther[i]));
}
function createMultihopSamples(params) {
    return params.inputsInEther.map((inputInEther, i) => {
        return {
            source: types_1.ERC20BridgeSource.MultiHop,
            fillData: {
                firstHopSource: {
                    source: params.firstHopSource,
                    fillData: {},
                },
                secondHopSource: {
                    source: params.secondHopSource,
                    fillData: {},
                },
                intermediateToken: 'fake-intermediate-token',
            },
            input: constants_1.ONE_ETHER.times(inputInEther),
            output: constants_1.ONE_ETHER.times(params.outputsInEther[i]),
        };
    });
}
function createOtcOrder(params) {
    const { inputInEther, outputInEther } = params;
    return {
        type: asset_swapper_1.FillQuoteTransformerOrderType.Otc,
        order: {
            txOrigin: '',
            expiryAndNonce: new utils_1.BigNumber(42),
            makerToken: 'fake-maker-token',
            takerToken: 'fake-taker-token',
            makerAmount: constants_1.ONE_ETHER.times(outputInEther),
            takerAmount: constants_1.ONE_ETHER.times(inputInEther),
            maker: 'maker',
            taker: 'taker',
            chainId: 42,
            verifyingContract: 'fake-verifying contract',
        },
        signature: {
            signatureType: asset_swapper_1.SignatureType.EIP712,
            r: 'r',
            s: 's',
            v: 42,
        },
        fillableMakerAmount: constants_1.ONE_ETHER.times(outputInEther),
        fillableTakerAmount: constants_1.ONE_ETHER.times(inputInEther),
        fillableTakerFeeAmount: new utils_1.BigNumber(0),
    };
}
//# sourceMappingURL=path_optimizer_test.js.map