...
import { GetOrderInfoWizardHandlers, CreateOrderWizardHandlers, CancelOrdersWizardHandlers, AllowanceWizardHandlers } from '../handlers/wizard_handlers';
...
export const createWizardRouter = (): express.Router => {
    ...
    const allow = new AllowanceWizardHandlers();
    /**
    ...
    router.post('/cancel', asyncHandler(cancelHandlers.constructStep1.bind(cancelHandlers)));
    router.get('/set_allowance', asyncHandler(allow.constructStep1.bind(allow)));
    return router;
};
