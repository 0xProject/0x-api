...
export class RfqmService {
    ...
    public async completeSubmissionLifecycleAsync(
        ...
        let tryResubmitting = true;
        while (!finishedMonitoring) {
            await sleep(TRANSACTION_WATCHER_SLEEP_TIME_MS);
                ...
                if (receipt.response !== undefined) {
                    ...
                    receipts.map((r) => {
                        if (r.response !== undefined) {
                            ...
                            submissions[r.transactionHash].status = RfqmTranasctionSubmissionStatus.Replaced;
                        }
                    });
